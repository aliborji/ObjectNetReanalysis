...
def motion_blur(x, severity=1):
    ...
    x = Image.fromarray(np.uint8(x)).convert('RGB') # BORJI
...
def fog(x, severity=1):
    ...
    tmp = c[0] * plasma_fractal(wibbledecay=c[1])[:64, :64][..., np.newaxis]     
    tmp = torch.tensor(cv2.resize(tmp, (224,224)))
    tmp = tmp[...,None]
    tmp = torch.cat([tmp, tmp, tmp],2)
    # import pdb; pdb.set_trace()
    x += tmp.numpy()
